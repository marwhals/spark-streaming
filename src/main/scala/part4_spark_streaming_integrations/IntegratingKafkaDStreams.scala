...
object IntegratingKafkaDStreams {
  ...
  val kafkaParams: Map[String, Object] = Map(
    ...
    "enable.auto.commit" -> false.asInstanceOf[Object] // won't compile without turning primitive into an object
  )
  ...
  def readFromKafka() = {
    ...
    val kafkaDStream = KafkaUtils.createDirectStream(
      ...
      /**
       ...
       - PreferFixed
      */
      ConsumerStrategies.Subscribe[String, String](topics, kafkaParams + ("group.id" -> "group1")) // For kafka cache requirements
      /*
        Alternative
  ...
  def writeToKafka() = {
    ...
    processedData.foreachRDD { rdd =>
      rdd.foreachPartition { partition =>
        ...
        // producer is available on this executor
  ...
  /**
   * Key points:
   * - Create the producer per partition as producers are no serializable
   * - Create a Kafka message per each record
   * - Close the producer when you are done processing the partition
   */
